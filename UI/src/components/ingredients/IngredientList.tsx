...
import { initDB, saveFavorite, removeFavorite, getFavorites } from "@/utils/indexedDB";
...
const IngredientList = ({ showOnlyFavorites = false }: IngredientListProps) => {
  ...
  const [showOnlyFavoritesState, setShowOnlyFavoritesState] = useState(showOnlyFavorites);
  ...
  const toggleFavorite = async (ingredient: Ingredient) => {
    try {
  ...
  const filteredIngredients = mockIngredients
    .filter(ingredient => {
      ...
      const matchesFavorites = !showOnlyFavoritesState || favorites.includes(ingredient.id);
    ...
    .sort((a, b) => {
      if (sortOrder === 'name') {
        ...
        // Sort by status priority: safe -> risky -> unknown -> risk
  ...
  return (
    <div className="w-full max-w-none space-y-4">
      <IngredientSearch
        ...
        onToggleFavorites={() => setShowOnlyFavoritesState(!showOnlyFavoritesState)}
      />
      ...
      <IngredientDetail
        ...
        isFavorite={selectedIngredient ? favorites.includes(selectedIngredient.id) : false}
        onToggleFavorite={toggleFavorite}
      />
    </div>
  );
